...
static off_t RecursiveInsert(BufferPool *pool,off_t T_addr, KeyType Key, int i, off_t Parent_addr, b_tree_row_row_cmp_t cmp) {
    ...
    if (T->n > Limit) {
        if (Parent_addr == -1) {
            ...
            if (Sibling_addr != -1) {
                Move(pool, T_addr, Sibling_addr, Parent_addr, i, 1,cmp,Key);
            }
            else {
                release(pool, T_addr);
                T_addr = SplitNode(pool, Parent_addr, T_addr, i);
...
static off_t SplitNode(BufferPool *pool, off_t Parent_addr, off_t X_addr, int i) {
    ...
    NewNode_addr = CreateNewNode(pool);
    ...
    if (Parent_addr != -1)
        ...
        Parent_addr = CreateNewNode(pool);
    ...
    if (SrcInFront) {
        if (Src->child[0] != -1) {
            while (j < n) {
                ...
                ReMove(pool,0, Src_addr, Child_addr, Src->n - 1, -1,key,cmp);
            ...
            while (j < n) {
                ...
                ReMove(pool,1, Parent_addr, Src_addr, i, Src->n - 1,key,cmp);
        ...
        if (Src->child[0] != -1) {
            while (j < n) {
                ...
                ReMove(pool,0, Src_addr, Child_addr, 0, -1,key,cmp);
            ...
            while (j < n) {
                ...
                ReMove(pool,1, Parent_addr, Src_addr, i, 0,key,cmp);
...
static off_t RecursiveRemove(BufferPool* pool, off_t T_addr, KeyType Key, int i, off_t Parent_addr, b_tree_row_row_cmp_t cmp) {
    ...
    if (T->child[0] == -1) {
        T_addr = ReMove(pool, 1, Parent_addr, T_addr, i, j,Key,cmp);
    }
    else {
    ...
    if (NeedAdjust) {
        if (Parent_addr == -1) {
            ...
            if (Sibling != -1) {
                Move(pool, Sibling, T_addr, Parent_addr, j, 1, cmp,Key);
            }
            else {
                BNode* Parent = (BNode*)get_page(pool, Parent_addr);
                if (i == 0)
...
static off_t MergeNode(BufferPool *pool, off_t Parent_addr, off_t X_addr, off_t S_addr, int i,b_tree_row_row_cmp_t cmp,KeyType key) {
    ...
    if (S->n > LIMIT_M_2) {
        ...
        Move(pool,S_addr, X_addr, Parent_addr, i, 1,cmp,key);
    }
    else {
        Limit = X->n;
        Move(pool, X_addr, S_addr, Parent_addr, i, Limit, cmp,key);
        ReMove(pool,0, Parent_addr, X_addr, i, -1,key,cmp);
