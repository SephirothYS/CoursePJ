    ...
    if (T->n > Limit) {
        ...
        if (Parent_addr == -1) {
            ...
            if (Sibling_addr != -1) {
                ...
                MoveElement(pool, T_addr, Sibling_addr, Parent_addr, i, 1,cmp);
            }
            else {
                /* 分裂节点 */
                release(pool, T_addr);
    ...
    if (SrcInFront) {
        if (Src->child[0] != -1) {
            while (j < n) {
                ...
                RemoveElement(pool,0, Src_addr, Child_addr, Src->n - 1, -1);
            ...
            while (j < n) {
                ...
                RemoveElement(pool,1, Parent_addr, Src_addr, i, Src->n - 1);
...
static off_t RecursiveRemove(BufferPool* pool, off_t T_addr, KeyType Key, int i, off_t Parent_addr, b_tree_row_row_cmp_t cmp) {
    ...
    if (T->child[0] == -1) {
        ...
        if (j == T->n || cmp(Key, T->row_ptr[j]) < 0) j--;



    /* 树叶 */
    if (T->child[0] == -1) {
        T_addr = RemoveElement(pool, 1, Parent_addr, T_addr, i, j);
    }
    else {
    ...
    if (NeedAdjust) {
        ...
        if (Parent_addr == -1) {
            ...
            if (Sibling != -1) {
                MoveElement(pool, Sibling, T_addr, Parent_addr, j, 1, cmp);
            }
            else {
                BNode* Parent = (BNode*)get_page(pool, Parent_addr);
                if (i == 0)
                ...
                release(pool, Parent_addr);
                Parent_addr = MergeNode(pool, Parent_addr, T_addr, Sibling, i, cmp);
...
static off_t MergeNode(BufferPool *pool, off_t Parent_addr, off_t X_addr, off_t S_addr, int i,b_tree_row_row_cmp_t cmp) {
    ...
    if (S->n > LIMIT_M_2) {
        /* 从S中移动一个元素到X中 */
        MoveElement(pool,S_addr, X_addr, Parent_addr, i, 1,cmp);
    }
    else {
        /* 将X全部元素移动到S中，并把X删除 */
        Limit = X->n;
        MoveElement(pool, X_addr, S_addr, Parent_addr, i, Limit, cmp);
        RemoveElement(pool,0, Parent_addr, X_addr, i, -1);
...
static off_t ReAdj(BufferPool* pool,off_t Parent_addr, off_t T_addr, KeyType Key, int i, b_tree_row_row_cmp_t cmp) {
    ...
    if (T->child[0] == -1) {
        ...
        if (Parent_addr != -1) {
            BNode* Parent = (BNode*)get_page(pool, Parent_addr);
            if (cmp(Parent->row_ptr[i], T->row_ptr[j]) < 0) {
                Parent->row_ptr[i] = T->row_ptr[j];
            }
            release(pool, Parent_addr);
        }
    }
