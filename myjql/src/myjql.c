...
RID insert_nonleaf_handler_t(RID rid) {};

void delete_nonleaf_handler_t(RID rid) {};

int rid_row_row_cmp(RID a, RID b) {
    if (get_rid_block_addr(a) == -1 && get_rid_block_addr(b) != -1)
        return -1;
    else if (get_rid_block_addr(a) != -1 && get_rid_block_addr(b) == -1)
        return 1;
    else if (get_rid_block_addr(a) == -1 && get_rid_block_addr(b) == -1)
        return 0;
    Record* rec_a,*rec_b;
    rec_a = (Record*)malloc(sizeof(Record));
    rec_b = (Record*)malloc(sizeof(Record));
    read_record(&tbl_rec,a,rec_a);
    read_record(&tbl_rec,a,rec_b);

    StringRecord* Str_a,*Str_b;
    Str_a = (StringRecord*)malloc(sizeof(StringRecord));
    Str_b = (StringRecord*)malloc(sizeof(StringRecord));
...
int rid_ptr_row_cmp(void *p, size_t size, RID b) {
    if (get_rid_block_addr(b) == -1) return 1;
    char* a = (char*)p;
    Record* rec;
    rec = (Record*)malloc(sizeof(Record));
    read_record(&tbl_rec,b,rec);
    StringRecord* Str;
    Str = (StringRecord*)malloc(sizeof(StringRecord));
...
size_t myjql_get(const char *key, size_t key_len, char *value, size_t max_size) {
    ...
    Str_rec = (StringRecord*)malloc(sizeof(StringRecord));
...
void myjql_set(const char *key, size_t key_len, const char *value, size_t value_len) {
    ...
    b_tree_insert(&bp_idx,b_tree_rid,&rid_row_row_cmp,&insert_nonleaf_handler_t);
...
void myjql_del(const char *key, size_t key_len) {
    ...
    b_tree_delete(&bp_idx,rec->key,&rid_row_row_cmp,&insert_nonleaf_handler_t,&delete_nonleaf_handler_t);
